...
connectToCamera()
setColorMode()
width, height = getAreaoOfInterest()
...
while(nRet == ueye.IS_SUCCESS):
    ...
    # Reshape the image in an numpy array
...
#---------------------------------------------------------------------------------------------------------------------------------------
    ...
    # Apply histogram equalization to creect the contrast
    hist = cv2.equalizeHist(gray)

    # Apply Gaussian blur to reduce noise
    blurred = cv2.GaussianBlur(hist, (31, 31), cv2.BORDER_DEFAULT)
    ...
    frame_as_text = base64.b64encode(frame)

    # Publish message to topic
    client.publish(topic, frame_as_text)
    print("Frame sent", end = "\r")
    #print(frame.shape)
...
#---------------------------------------------------------------------------------------------------------------------------------------
    ...
    #cv2.imshow("CoinDetection", frame)
